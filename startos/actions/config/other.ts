...
const configSpec = sdk.InputSpec.of({
  ...
  blocknotify: Value.text({
    ...
    required: false,
    default: null,
    description: i18n('Execute an arbitrary command when the best block changes'),
  }),
  ...
  enableIpc: Value.dynamicToggle(async ({ effects }) => {
    ...
    return {
      ...
      warning: ipcEnabled
        ? null
        : i18n('IPC is an experimental feature. Only enable this if you know what you are doing with the IPC socket. An example use case would be Stratum v2 mining services.'),
      default: false,
    }
  }),
})
