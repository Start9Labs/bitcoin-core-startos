...
- The Private Broadcast conflict check now weighs Onlynet as it will actually be written. With the SAM proxy off, a selection of i2p plus another network used to pass the check and then leave Private Broadcast with no Tor or I2P route at all, which Bitcoin also refuses to start on.`,
...
- La comprobación de conflictos de la Transmisión privada ahora tiene en cuenta Onlynet tal como se va a escribir. Con el proxy SAM desactivado, seleccionar i2p junto con otra red pasaba la comprobación y dejaba a la Transmisión privada sin ninguna ruta Tor ni I2P, algo con lo que Bitcoin tampoco arranca.`,
...
- Die Konfliktprüfung für Privater Broadcast bewertet Onlynet jetzt so, wie es tatsächlich geschrieben wird. Bei abgeschaltetem SAM-Proxy kam eine Auswahl aus i2p und einem weiteren Netzwerk durch die Prüfung und ließ Privater Broadcast ohne jede Tor- oder I2P-Route zurück — auch damit startet Bitcoin nicht.`,
...
- Kontrola konfliktów Prywatnego rozgłaszania uwzględnia teraz Onlynet w postaci, w jakiej zostanie zapisany. Przy wyłączonym proxy SAM wybór i2p wraz z inną siecią przechodził kontrolę i zostawiał Prywatne rozgłaszanie bez żadnej trasy przez Tora ani I2P, na czym Bitcoin również się nie uruchamia.`,
...
- La vérification des conflits de la Diffusion privée tient désormais compte d'Onlynet tel qu'il sera réellement écrit. Avec le proxy SAM désactivé, sélectionner i2p en plus d'un autre réseau passait la vérification et laissait la Diffusion privée sans aucune route Tor ni I2P, ce sur quoi Bitcoin refuse également de démarrer.`,
  },
  migrations: {
    up: async ({ effects }) => {},
    down: async ({ effects }) => {
